...
Clinical risk factors including male sex, diabetes, and hepatitis B infection have been associated with poor survival rates. Scoring systems such as the Barcelona Clinic Liver cancer system, endorsed by the European Association for the Study of the Liver and the American Association for the Study of Liver Diseases, incorporate the extent to the primary lesion, performance status, and liver function in prognostication. Additionally, new tumor molecular biomarkers is increasingly being incorporated into some classifications and represent promising tools for prognosis prediction.

Given the limited health care resources and the positive association between aggressive treatment of improvement of outcomes, better understanding the incremental predictive performance of these approaches will guide the development of the optimal screening guidelines.
...
4. The models were evaluated to eliminate if present multicollinearity, autocorrelation, and influential points
...
* 98 (60.9%) of the population lives
...
* are **lower risk as indicated by clinical factors**: slightly younger (p = 0.035), have a lower proportion of diabetes (non-sig), have a lower proportion with Hepatitis C Virus Antibody (non-sig), have a lower proportion presenting symptoms (p = 0.001)
* have **better tumor characteristics**: smaller nodule dimension (p > 0.030), have a lower propotion with metastatic tumour (p = 0.003)
* have **better liver function**: have a lower total bilirubin (p = 0.031), have a lower INR (p = 0.029), lower hemoglobin (p < 0.001), lower degree of ascites (p = 0.004), lower proportion with encephalopathy (p = 0.038), lower AST (p = 0.002), lower ALP (p < 0.001), lower proportion with PVT (p = 0.013)
...
`randomForest` used to select predictors for `bestglm` and `glmulti`
...
dataClinical.Xy <- dataClinical %>%
    ...
    mutate_all(as.numeric) %>%
    rename(y = lives) %>%
    dplyr::select(-c(protein_total, alcohol, hepCvAb, obesity))  # max p = 15
...
Selected the top 10 predictors as per random forest for `glmulti`
...
* The model selected via automated model selection and model-averaging is the same as best subsets
...
----------- | ---------------- | ----------------
Accuracy    | 0.72             | 0.72            
Concordance | 0.77             | 0.75            
AUC         | 0.77 [0.68-0.85] | 0.75 [0.66-0.84]
PPV         | 0.69             | 0.69
NPV         | 0.89             | 0.94

* Clinical model B has slighter higher NPV
...
Model       | A                | B                | C
----------- | ---------------- | ---------------- | -----
Accuracy    | 0.78             | 0.76             | 0.72
Concordance | 0.85             | 0.81             | 0.77
AUC         | 0.85 [0.78-0.92] | 0.81 [0.73-0.88] | 0.77 [0.69 - 0.85]
PPV         | 0.85             | 0.91             | 0.88
NPV         | 0.69             | 0.64             | 0.59

* Tumor/liver model A has slightly higher accuracy, concordance, and discrimination as per AUC
* Tumor/liver model B has a slightly higher PPV which would mean the fewest false positives
...
Model       | A                | B                | C
----------- | ---------------- | ---------------- | -----
Accuracy    | 0.40             | 0.64             | 0.64
Concordance | 0.26             | 0.58             | 0.59
AUC         | 0.73 [0.64-0.83] | 0.60 [0.50-0.70] | 0.58 [0.48 - 0.69]
PPV         | 1.00             | 0.67             | 0.66
NPV         | 0.40             | 0.58             | 0.57

* Tumor/liver model B and C have a higher accuracy and concordance but the AUC isn't statistically significant and have a low PPV
* Tumor/liver model B has the highest PPV and a statistically significant AUC
...
* As per the likelihood ratio test, the model including both clinical risk factors and assessments of tumor staging and liver function fit the data better than the clinical risk factors alone
* AFP doesn't substantially improve model fit
...
Model       | C    | CB   | CBM
----------- | ---- | -----| -----
Accuracy    | 0.72 | 0.85 | 0.86
Concordance | 0.75 | 0.90 | 0.90
AUC         | 0.75 | 0.90 | 0.90
PPV         | 0.69 | 0.85 | 0.90
NPV         | 0.94 | 0.86 | 0.81
...
* This isn't substantially improved with AFP
...
Model       | C    | CB   | CBM
----------- | ---- | -----| -----
Accuracy    | 0.58 | 0.81 | 0.81
Concordance | 0.61 | 0.77 | 0.77
AUC         | 0.61 | 0.78 | 0.77
PPV         | 0.85 | 0.84 | 0.84
NPV         | 0.55 | 0.75 | 0.75
...
* The PPV and NPV did not decrease with the same magnitude
* Tumor staging and liver function assessments still appear to improve model fit for survival
...
* 10-fold cross validation of the model consisting of clinical risk factors as well as tumor staging and liver function assessments showed an accuracy of 80%, PPV 80% and NPV 82%
* This performance doesn't substantially differ from the training and test datasets

Model       | C - training  | C - test  | C - CV 
----------- | ------------- | ----------| -------
Accuracy    | 0.72          | 0.58      | 0.64 
PPV         | 0.69          | 0.85      | 0.59 
NPV         | 0.94          | 0.55      | 0.71 


Model       | CB - training | CB - test | CB - CV
----------- | ------------- | ----------| -------
Accuracy    | 0.85          | 0.81      | 0.80   
PPV         | 0.85          | 0.84      | 0.80 
NPV         | 0.86          | 0.75      | 0.82 
...
* As illustrated by the nomogram, given a prevalence of HCC survival of 60.9% (pre-test probability), a positive test result with a model consisting of clinical risk factors only would provide a post-test probability of 77.9%
* A model consisting a clinical risk factors as well as tumor staging and liver function assessments would provide a post-test probability of 90.8%

# Conclusion

The predictive ability of clinical risk factors to identify survival among patients diagnosed with hepatocellular carcinoma (HCC) was improved with tumor staging and liver function assessments. However, incorporation of molecular biomarkers is not expected to substantially improve HCC prognostication.
